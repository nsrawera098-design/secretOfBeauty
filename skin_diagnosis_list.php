...
$query = "SELECT * FROM skin_diagnosis $where_sql ORDER BY created_at DESC";
...
<head>
    ...
    <style>
        ...
        a.button { background-color: #ff69b4; color: white; padding: 6px 10px; text-decoration: none; border-radius: 5px; }
        a.button:hover { background-color: #ff1493; }
...
<?php if (mysqli_num_rows($result) > 0): ?>
    <?php while ($row = mysqli_fetch_assoc($result)): ?>
        <tr>
            ...
            <td><?= htmlspecialchars($row['skin_issues']); ?></td>
            <td>
                <?php if (!empty($row['face_image'])): ?>
                    <img src="<?= htmlspecialchars($row['face_image']); ?>" alt="תמונה">
                <?php else: ?>
                    אין תמונה
                <?php endif; ?>
            </td>
...
<?php else: ?>
    <tr>
        <td colspan="7">אין אבחונים במערכת בטווח התאריכים שבחרת.</td>
    </tr>
<?php endif; ?>
</tbody>
</table>
